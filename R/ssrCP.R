...
condPower <- function(z1, n2, z2 = z2NC, theta = NULL,
                      x = gsDesignCRT(k = 2, timing = .5, beta = beta),
                      ...) {
...
#' \code{\link{gsDesignCRT}}. For \code{plot.ssrCP}, \code{x} is a design returned
#' by \code{ssrCP()}.
...
#' \code{\link{gsDesignCRT}} for a description of how this is related to
#' \code{theta}.
...
#' x <- gsDesignCRT(k=2,n.fix=n.fix,alpha=alpha,test.type=1,sfu=sfHSD,
...
ssrCP <- function(z1, theta = NULL, maxinc = 2, overrun = 0,
  ...
  if (!inherits(x, "gsDesignCRT")) {
    stop("x must be passed as an object of class gsDesignCRT")
  }
  if (2 != x$k) {
