...
#' @param ifrac_size_m Number of clusters at each interim look
#' @param ifrac_size_n Average cluster size at each interim look
...
gsSimContCRT <- function(t, data, test_type, recruit_type, icc_type, stat_type,
                         delta, sigma_vec, rho, alpha = 0.05, beta = 0.1,
  ...
  while (!stop && ta_i < t) {
    ...
    if (stat_type == 1) {
      se <- seContDiff(x1_i, x2_i, m_i, n_i, rho_i, sigma_vec)
    } else {
      se <- seContDiff(x1_i, x2_i, m_i, n_i, rho_i, NULL)
    }
...
seContDiff <- function(x1_i, x2_i, m, n, rho, sigma_vec = NULL) {
  if (is.null(sigma_vec)) {
    pooled <- ((m * n - 1) * var(x1_i) +
                 (m * n - 1) * var(x2_i)) / (2 * m * n - 2)
  } else {
    pooled <- ((m * n - 1) * sigma_vec[1]^2 +
                 (m * n - 1) * sigma_vec[2]^2) / (2 * m * n - 2)
  }
  se <- sqrt(2 * pooled * (1 + (n - 1) * rho) / (m * n))
  return(se)
}