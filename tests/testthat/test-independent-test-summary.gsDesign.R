# Testing summary.gsDesignCRT function for different functions and test.types.
...
testthat::test_that(desc = "Test - gsSurv", code = {
  ...
  local_edition(3) # use 3rd edition of testthat for this testcase
  expect_snapshot_output(x = summary.gsDesignCRT(xgs))
})
...
testthat::test_that(desc = "Test gsDesignCRT summary for gsSurv - error", code = {
  local_edition(3) # use 3rd edition of testthat for this testcase
  expect_error(summary.gsDesignCRT(gsSurv(
...
testthat::test_that(
  desc = "Test gsDesignCRT summary for gsSurv - error - nlambda ",
  code = {
    local_edition(3) # use 3rd edition of testthat for this testcase
    expect_error(summary.gsDesignCRT(gsSurv(
...
testthat::test_that(desc = "Test:  nSurvival object", code = {
  ...
  local_edition(3) # use 3rd edition of testthat for this testcase
  expect_snapshot_output(x = summary.gsDesignCRT(xs))
})



testthat::test_that(desc = "Test gsDesignCRT summary for nSurvival", code = {
  ss <- nSurvival(
    lambda1 = .2, lambda2 = .1, eta = .1, Ts = 2, Tr = .5,
...
testthat::test_that(desc = "Test:  nBinomial object", code = {
  ...
  local_edition(3) # use 3rd edition of testthat for this testcase
  expect_snapshot_output(x = summary.gsDesignCRT(xp))
})


testthat::test_that(
  desc = "Test gsDesignCRT summary -test.type = 2",
  code = {
    xgs <- gsDesignCRT(k = 4, test.type = 2, sfu = "Pocock")
    local_edition(3) # use 3rd edition of testthat for this testcase
    expect_snapshot_output(x = summary.gsDesignCRT(xgs))
  }
)
