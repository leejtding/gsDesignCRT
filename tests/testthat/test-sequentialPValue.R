testthat::test_that("sequential p-value not computed correctly",{
  x <- gsDesignCRT(n.fix=100,k=4,timing=c(.4,.7,.85))
  interval <- c(.001,.999)
  # Check numbers correctly generated when end of interval is appropriate at IA
  seqp <- sequentialPValue(gsD=gsDesignCRT(n.fix=100,k=4,timing=c(.4,.7,.85)),Z=5,n.I=44,interval=c(.001,.999))
  ...
  expect_equal(min(gsDesignCRT(alpha=seqp,k=x$k,maxn.IPlan=max(x$n.I),n.I=n.I,
