# gsDesignCRT 3.5.0 (July, 2023)

- `sfPower()` now allows a wider parameter range (0, 15].
- `toInteger()` function added to convert `gsDesignCRT` or `gsSurv` classes
...
- Link update for the gsDesignCRT manual in the documentation,
  now directly pointing to the gsDesignCRT technical manual bookdown project.
- Introduced a new hex sticker logo.

# gsDesignCRT 3.4.0 (October, 2022)
...
# gsDesignCRT 3.2.2 (January, 2022)
...
# gsDesignCRT 3.2.0 (January, 2021)
...
- Backwards compatible addition of spending time capabilities to `gsDesignCRT` and `gsSurv`

# gsDesignCRT 3.0-5 (January, 2018)

- Registered C routines
- Fixed "gsbound"
- Replaced "array" by "rep" calls to avoid `R CMD check` warnings

# gsDesignCRT 3.0-4 (September, 2017)
...
- Introduced spending time as a separate concept from information time to enable concepts such as calendar-based spending functions. The only user function changed is the `gsDesignCRT()` function and the change is the addition of the parameters `usTime` and `lsTime`; default behavior is backwards compatible.

# gsDesignCRT 3.0-2 (February, 2016)
...
# gsDesignCRT 3.0-1 (January, 2016)
...
# gsDesignCRT 3.0-0 (December, 2015)
...
# gsDesignCRT 2.9-3 (November, 2014)
...
- Added `summary.spendfn()` to provide textual summary of spending functions; this simplified the print function for gsDesignCRT objects
...
- Fixed rounding so that `gsBoundSummary`, `xtable.gsSurv` and `summary.gsDesignCRT` are consistent for `gsSurv` objects
