...
Several plots are available to summarize a design; see help for `plot.gsDesignCRT()`; one easy way to see how to generate each is by checking plots and code generated by the [Shiny interface](https://gsdesign.shinyapps.io/prod/).
...
We specify a normal prior for the standardized effect size using the `gsDesignCRT::normalGrid()` function.
