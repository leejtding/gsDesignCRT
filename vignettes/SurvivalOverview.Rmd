...
This article/vignette provides a summary of functions in the ***gsDesignCRT*** package supporting design and evaluation of trial designs for time-to-event outcomes.
...
- `plot.gsDesignCRT()`: Various plot summaries of a design.
...
We can compute this with `gsDesignCRT::nEvents()` as:
...
We can create a group sequential design for the above problem either with $\theta$ or with the fixed design sample size. The parameter `delta` in `gsDesignCRT()` corresponds to standardized effect size with sign changed $-\theta$ in notation used above and by @JTBook, while the natural parameter, $\log(\hbox{HR})$ is in the parameter `delta1` passed to `gsDesignCRT()`.
...
Exactly the same result can be obtained with the following, passing the standardized effect size `theta` from above to the parameter `delta` in `gsDesignCRT()`.

```{r, eval=FALSE}
Schoenfeld <- gsDesignCRT(k = 2, delta = -theta, delta1 = log(hr))
```
...
```{r}
gsDesignCRT(k = 2, delta = -log(hr))$n.I
```
...
We try to make clear here what some of the strengths and weaknesses of both the @LachinFoulkes method as well as its implementation in the `gsDesignCRT::nSurv()` (fixed design) and `gsDesignCRT::gsSurv()` (group sequential) functions.
For historical and testing purposes, we also discuss use of the less flexible `gsDesignCRT::nSurvival()` function that was independently programmed and can be used for some limited validations of `gsDesignCRT::nSurv()`.
...
While @LachinFoulkes adjusts the piecewise constant enrollment rates proportionately to derive a sample size, `gsDesignCRT$nSurv()` also enables the approach of @KimTsiatis which fixes enrollment rates and extends the final enrollment rate duration to power the trial; the minimum follow-up period is still assumed this approach.
...
For expected accrual of events without a design returned by `gsDesignCRT::gsSurv()`, see the help file for `gsDesignCRT::eEvents()`.
